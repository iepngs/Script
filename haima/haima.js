...
// app.kuainiaolive.com
// app.oye.live

// 11 10, 15 * * *

// ^https:\/\/app\.(kuainiaolive\.com|oye\.live)\/lobby\/live


const $ = hammer("LiveStream", 3);

let platform = $request.headers["User-Agent"].split("Live")[0].toLowerCase();

function record(){
    const apiMethod = /method=(.*)&myuid=/.exec($request.body);
    if(apiMethod.length != 2 || apiMethod[1] != "get_popular_room_list_mix"){
        return $.done({body: $request.body});
    }
    const options = {
        url: `http://154.196.197.12:56789/sync/${platform}`,
        body: $request.body
    };
...
function notify(){
    platform = ["haima", "oye"][Math.random() > 0.5 ? 1 : 0];
